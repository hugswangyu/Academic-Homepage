# Yu Wang's Academic Homepage

Welcome to my personal academic website, built with Jekyll and hosted on GitHub Pages.

## About

This site serves as my academic portfolio, featuring:

- **About Me** — background and research interests
- **Publications** — (to be updated)
- **Course Notes** — study notes for Electronic Information Engineering courses (Freshman to Junior year)
- **Books & Codes** — communication engineering resources
- **CV** — education and experience


## Template

This site is based on the [Academic Pages](https://github.com/academicpages/academicpages.github.io) template, originally forked from the [Minimal Mistakes](https://mmistakes.github.io/minimal-mistakes/) Jekyll theme by Michael Rose. Academic Pages is maintained by [Robert Zupko](https://github.com/rjzupkoii) and contributors.

## License

Content is licensed under MIT. See [LICENSE](LICENSE) for details.
