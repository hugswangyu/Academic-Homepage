---
layout: single 
title: "Junior Year" 
permalink: /course-notes/junior/ 
author_profile: true 
---


This section includes my course notes and recommended online learning resources from my junior year.

---

## 数字信号处理 Digital Signal Processing

### 概述
数字信号处理是电子信息类核心专业课程，涵盖时域与频域分析、离散傅里叶变换、FFT、IIR/FIR滤波器设计、多速率信号处理等内容，是通信、雷达、图像处理等方向的理论基础。

### 学习资源

* **在线课程：** 

* **我的笔记：** [数字信号处理笔记]

---

## 通信原理（双语版）Principles of Communication, Bilingual Version

### 概述
通信原理主要讲解通信系统基本模型、随机过程、模拟调制、数字基带传输、数字频带调制、信道容量、差错控制编码等核心理论，双语授课兼顾专业英文术语与理论体系，为后续通信类科研与工程应用奠定基础。

### 学习资源

* **在线课程：** 

* **我的笔记：** <br>
[通信原理笔记] <br>
[全书题库及新技术考点]<br>
[midterm cheating sheet] 

---

## 高频电子线路 High-Frequency Electronic Circuits

### 概述
高频电子线路聚焦射频/高频领域，涵盖谐振回路、高频小信号放大器、功率放大器、正弦波振荡器、调制与解调、锁相环等模块，是无线通信、射频电路设计的前置核心课程。

### 学习资源
* **在线课程：** 

* **我的笔记：** [高频电子线路笔记]
  
---

## 信息论 Information Theory

### 概述
信息论以信息度量为核心，包括信源熵、互信息、信道容量、香农定理、信源编码、信道编码基础等内容，从理论层面揭示信息传输与压缩的极限，是通信、编码、人工智能领域的理论基石。

### 学习资源
* **在线课程：** 

* **我的笔记：** [信息论笔记]

---

## 数据通信与计算机网络 Data Communications and Computer Networks

### 概述
本课程融合数据通信原理与计算机网络体系结构，涵盖物理层、数据链路层、网络层、传输层、应用层协议，以及差错控制、多路复用、路由算法、网络安全等内容，兼顾通信底层传输与互联网协议架构。

### 学习资源
* **在线课程：** 

* **我的笔记：** <br>
[计网笔记] <br>
[计网知识点]
